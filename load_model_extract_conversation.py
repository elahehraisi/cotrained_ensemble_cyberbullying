...
# this script is to load the message and user learners, and find the top bullying conversations according to that models

# ####################################################################
# ######**************** load the trained model and extract top 100 conversations
# ####################################################################
...
parser.add_argument('--user_classifier', type=str, default='node2vec',
...
load_dir = './data/' + args.data_set + '/saved_data/' + args.message_classifier + '_' + args.user_classifier + '/'+ args.batch_normalization + '/dropout_' + dropout + '/'
...
# # ####################################################################
# # ######**************** load the last saved message and user learners
# # ####################################################################

message_model,all_message = utils.load_saved_message_learner(args,pretrained,load_dir)
user_model,all_user = utils.load_saved_user_learner(args,pretrained,load_dir)
#
# # ####################################################################
# # ######**************** validation on all data to get the score of all messages
# # ####################################################################
...
def validate(validation_data,save_score=False):
    ...
    for indx in range(iter):
        ...
        if (x_message.size())[0] == args.batch_size:
            ...
            if args.message_classifier == 'rnn':
                y_msg, hidden = message_model(x_message.t(), hidden)
                y_mssg.append(y_msg)
            ...
            if save_score == True:
                if indx == 0:
                    ...
                    if args.user_classifier != 'none':
                        ...
                        y_u = torch.cat((y_u, y_user))
                        y_m = torch.cat((y_m, y_msg))
                    else:
    ...
    if save_score == True:
        ...
        validation_loss['y_user'] = y_u
        validation_loss['y_message'] = y_m
...
try:
    ...
    y_u = validation_loss['y_user']
    y_m = validation_loss['y_message']

    # use the score of users and message in order to extract top bullying user pairs
    # a file user_pair_indices.txt will be saved in load_dir. It contains user pairs
    # sorted from high to low based on bullying score
    utils.get_top_user_pairs(args,score,load_dir)
